...
function db = prepare_dreemdb(src,feature_fun,X,opt)
	if nargin < 3
		opt = struct();
	end
	N = size(X,1);
	...
	if opt.parallel
		...
		parfor k = 1 : N
            disp(['runing parfor ith example... i = ', int2str(k)]);    
		...
		for k = 1:N
			...
			estimated_time_left = time_elapsed * (N-k) / k;
			fprintf('.');
		end
	end
